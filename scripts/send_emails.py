...
from getmehired.services.storage import load, load_recruiters
...
async def main(
    ...
    from getmehired.services.gmail_sender import _is_sendable
    ...
    # ── STEP 3: Check bounces (if --retry-bounced or --check-bounces) ─────────
    settings = get_settings()

    if retry_bounced or check_bounces_only:
        _section("STEP 3 — Check Bounces (pre-send)")
    ...
    if not check_bounces_only:
        _section(f"STEP {'4' if retry_bounced else '3'} — Send Emails{'  [DRY RUN]' if dry_run else ''}")

        t0 = time.perf_counter()
        try:
            recruiters = await send_batch(
                job_path, service, max_send=max_send, dry_run=dry_run, from_name=from_name
            )
            elapsed = time.perf_counter() - t0
